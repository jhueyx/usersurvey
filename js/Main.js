...
function submitForm() {
	// select all elements with class="question1" classified as checks to see which boxes were checked off
	var checks  = document.getElementsByClassName("question1");
	// select all elements with class="question2" , classfied as the selection from the dropdown
	var selects = document.getElementsByClassName("question2");
	// select all elements with class="question2", classiifed as the selection from the radio selection
	...
	// create variables to eventually hold the users answers- variables for each answer
	...
	// if there are no checkboxes selectred then the value is 0, then return the failure message
	if(answer1.length == 0) {
